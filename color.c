...
RGB
getColorUnderPointer(Display *display, Window window)
{
    RGB rgb;
    ...
    XQueryPointer(display, window, &root_return, &child_return, \
            &root_x_return, &root_y_return, &child_x_return, \
            &child_y_return, &mask_return);

    if (child_return != None)
        image = XGetImage(display, child_return, \
            child_x_return, child_y_return, 1, 1, AllPlanes, ZPixmap);
    else
        image = XGetImage(display, root_return, \
            root_x_return, root_y_return, 1, 1, AllPlanes, ZPixmap);
...
main(int argc, char *argv[])
{
    ...
    XGrabPointer(display, root_window, False, event_mask, GrabModeAsync, \
